...
describe('testing updating record with PUT /content/:id', () => {
  ...
  test('testing updating content successfully', async done => {
    ...
    expect(getUpdatedRecord.body.title).toEqual('test title (testing update record) UPDATED') // checking body title
    expect(getUpdatedRecord.body.content).toEqual('test content (testing update record) UPDATED') // checking body content
    expect(getUpdatedRecord.body.date_created).toEqual('2021-08-11T14:43:31.938Z') // checking date created
    done()
  })
